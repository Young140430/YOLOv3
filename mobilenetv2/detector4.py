...
def Net_out(image_data_):
    net_ = MobileNet()
    net_.load_state_dict(torch.load(r"E:\PycharmProjects\pythonProject1\2022.01\p20220110\yolov3\param\voc_best5-1.pt"))
    net_.eval()
...
if __name__ == '__main__':
    ...
    text01 = open(r"E:\VOC2012\train.txt")
    cls_names = text01.readlines()
    text01.close()

    path = r"E:\voc2007\VOCtest_06-Nov-2007\VOCdevkit\VOC2007\JPEGImages"
    for img_file in glob.glob("{}/*.jpg".format(path)):
        ...
        idxs_13, vecs_13 = get_offsets(output_13, 0.08)
        vecs_13[..., 5:] = torch.softmax(vecs_13[..., 5:], dim=1)
        boxes_13 = get_boxes(idxs_13, vecs_13, 32, resize_h, cfg2.ANCHORS_GROUP[13])

        idxs_26, vecs_26 = get_offsets(output_26, 0.08)
        vecs_26[..., 5:] = torch.softmax(vecs_26[..., 5:], dim=1)
        boxes_26 = get_boxes(idxs_26, vecs_26, 16, resize_h, cfg2.ANCHORS_GROUP[26])

        idxs_52, vecs_52 = get_offsets(output_52, 0.08)
        vecs_52[..., 5:] = torch.softmax(vecs_52[..., 5:], dim=1)
        boxes_52 = get_boxes(idxs_52, vecs_52, 8, resize_h, cfg2.ANCHORS_GROUP[52])
        ...
        for box_ in box:
            ...
            cv2.putText(image, cls_names[cls_index][:-1], (x1, y1+10), cv2.FONT_HERSHEY_SIMPLEX, 0.35, (0, 0, 255))
