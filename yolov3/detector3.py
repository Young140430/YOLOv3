...
def Net_out(image_data_):
    net_ = Darknet53()
    net_.load_state_dict(torch.load(r"E:\yolov3_param\net_best.pt"))
    net_.eval()
...
if __name__ == '__main__':
    ...
    text01 = open(r"E:\train.txt")
    cls_names = text01.readlines()
    text01.close()

    path = r"E:\yolov3_train"
    for img_file in glob.glob("{}/*.jpg".format(path)):
        ...
        boxes_13 = get_boxes(idxs_13, vecs_13, 32, resize_h, cfg.ANCHORS_GROUP[13])

        idxs_26, vecs_26 = get_offsets(output_26, 0.1)
        vecs_26[..., 5:] = torch.softmax(vecs_26[..., 5:], dim=1)
        boxes_26 = get_boxes(idxs_26, vecs_26, 16, resize_h, cfg.ANCHORS_GROUP[26])

        idxs_52, vecs_52 = get_offsets(output_52, 0.1)
        vecs_52[..., 5:] = torch.softmax(vecs_52[..., 5:], dim=1)
        boxes_52 = get_boxes(idxs_52, vecs_52, 8, resize_h, cfg.ANCHORS_GROUP[52])
